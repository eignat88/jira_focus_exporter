...
class ParallelLoaderV2:
    ...
    def load_table(self, table_name: str, columns: Optional[List[str]] = None,
        ...
        # PostgreSQL — таблицы в нижнем регистре
        ...
        try:
            ...
            self.start_threads(source_table, target_table, source_schema, ss_columns, pg_columns,
    ...
    def start_threads(self, source_table, target_table, source_schema, ss_columns, pg_columns,
                      chunk_column, chunk_index, run_id, total_chunks):
        writer = threading.Thread(
            target=self.writer,
            args=(target_table, pg_columns, run_id, total_chunks),
            daemon=True,
        )
        ...
        for idx in range(self.workers):
            thread = threading.Thread(
                ...
                args=(idx, source_table, source_schema, ss_columns,
    ...
    def target_column_exists(self, conn, table_name: str, column_name: str) -> bool:
        """Return True when the target RAW table contains the requested column."""
        with conn.cursor() as cur:
            cur.execute(
                """
                SELECT EXISTS (
                ...
                """,
                (self.pg.schema, table_name, column_name),
            )
            return bool(cur.fetchone()[0])

    def finalize_chunk(self, conn, staging, target_table, columns, item, run_id):
        insert_columns = list(columns)
        select_expressions = [ident(c) for c in columns]

        # Populate the normalized numeric RECID during SQL Server -> RAW loading.
        # The extra expression is enabled only when:
        #   1. the target RAW table has recid_bigint;
        #   2. the source/staging projection contains recid;
        #   3. recid_bigint is not already supplied by the source.
        #
        # Invalid/empty RECID values are converted to NULL instead of aborting
        # the whole chunk. Data-quality checks can then detect them explicitly.
        lower_columns = {c.lower() for c in columns}
        if (
            "recid" in lower_columns
            and "recid_bigint" not in lower_columns
            and self.target_column_exists(conn, target_table, "recid_bigint")
        ):
            insert_columns.append("recid_bigint")
            select_expressions.append(
                "CASE "
                "WHEN BTRIM(" + ident("recid") + ") ~ '^[0-9]+$' "
                "THEN BTRIM(" + ident("recid") + ")::bigint "
                "ELSE NULL END"
            )

        insert_cols_sql = ", ".join(ident(c) for c in insert_columns)
        select_cols_sql = ", ".join(select_expressions)
        ...
        with conn.cursor() as cur:
            ...
            cur.execute(
                f"INSERT INTO {ident(self.pg.schema)}.{ident(target_table)} "
                f"({insert_cols_sql}) "
                f"SELECT {select_cols_sql} "
                f"FROM {ident(self.pg.schema)}.{ident(staging)} "
